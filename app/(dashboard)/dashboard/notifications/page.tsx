...
import { fetchAdminNotifications, markAdminNotificationsRead } from "@/lib/estate-api";
...
function mapAdminNotification(doc: Record<string, unknown>): AdminNotificationRow {
  return {
    id: String(doc._id ?? doc.id ?? ""),
    message: String(doc.message ?? ""),
    timeLabel: doc.timeLabel ? String(doc.timeLabel) : undefined,
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        if (isApiMode()) {
          ...
          setAdminNotifications(rows.map((r) => mapAdminNotification(r as Record<string, unknown>)));
          setNotifications([]);
          return;
        }
    ...
    const onStorage = (e: StorageEvent) => {
      if (!isApiMode() && e.key === "estateos_resident_notifications_v1") {
        setNotifications(loadNotifications());
      }
    };
  ...
  const unreadCount = useMemo(() => {
    if (isApiMode()) return adminNotifications.filter((n) => !n.read).length;
            ...
            adminNotifications.map((n) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-foreground">{n.type ?? "Estate notice"}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{n.message}</p>
                  {n.timeLabel && <p className="text-xs text-muted-foreground mt-1">{n.timeLabel}</p>}
                </div>
